...
- **Release rights (the baseline release-guard, transplanted)**:
  ...
  `QUERY_RESULT_RECORDED(REJECTED)` only, `BUSINESS_REJECT` never
...
- **Feed matching (fail-closed multiplicity, counted in PAYMENTS not
  ...
  path, and unmatched TERMINAL-class evidence is inbox-recorded AND
  paged to ops, never silently acked; 1 → fold + append under the
  lock; 2+ DISTINCT payments → CRITICAL anomaly, NO state change on
  any stream.
...
- **Archival is DESIGNED but far-future** (`event-model-v2.md` §9):
  ...
  upstream-reprocessing + engine-key-retention + EVERY evidence
  channel's lateness bound (a late settlement must still find its
  UETR); archive storage stays FOLD-READABLE (deploy gate and feed
  matching cover archived streams); rehydration

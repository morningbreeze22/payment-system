...
1. **Adversarial review rounds.** Twenty-eight external adversarial
   ...
   invalid marker scoped to its durable seen-admission set
   (later-introduced payments no longer inherit it) — all closed by
   mechanism or removal, see `event-model-v2.md` §0; no clean round
   yet after twenty-eight).

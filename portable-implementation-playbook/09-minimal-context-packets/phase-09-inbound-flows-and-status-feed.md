...
Read: §6.1 (ADMISSION first — round 5; block TRADE-SNAPSHOT FENCE — rounds 6–7) §2.4 §6.7 (whole) §6.9 (required row) §6.0. Invariant: NO per-block work before the trade-level admission transaction (upsert-lock trade_snapshot_state; newer → admit+update; equal+digest-equal → admit; equal+digest-differs → tie alert, stop; older → refuse WHOLE — a refused document NEVER creates a scope); EVERY block transaction locks the trade row FIRST and re-verifies the admitted (ordering, digest) — the FENCE (never "currency": that is a scope-key field) — mismatch → STOP fan-out (§6.1 block-level supersession; abandoned blocks logged + counted; never a check outside the lock); global lock order trade row → obligation → request; ack only after the fan-out completes; required_amount mutates only on strictly-newer ordering; comparator is ONE pluggable point shared by admission and blocks; ABSENT obligations (round 11): after the fan-out, zero each obligation NOT in the document (required := 0 AND upstream_ordering := doc.ordering; fence + lock + strictly-newer) — cleanly unwound rows derive §4.1 CANCELLED; anchors zeroed ONLY when doc.ordering > validation_failed_ordering; an EMPTY derived set (§6.0 role derivation) is VALID — pure-absence fan-out (only-payment removal); every zeroing fan-out emits the §15 disappearance log+metric (tuples MASKED per §16.3 — never a raw debit_account).
Placeholders: [Obligation Repository]. Mappings: upsert path; B-01 answered; S-10 applied.
Objective: admission gate + per-block trade-snapshot fence; locked upsert (ORA-00001 retry); guard; stale counted; T1 → RG-06 even without amount change.
Tests: §6.7 regression trace; ties at admission; T1; T-35 (never-seen-scope refused; disjoint first snapshots serialize; pause-after-admission/after-block-1 overtake → next block aborts; failed-validation advances neither watermark); T-37 absence-lifecycle set (BLOCKING — empty derived set, only-payment removal, CANCELLED terminal, anchor ordering-aware retirement, reappearance, redelivery convergence). Stop: merged.
```

...
Objective: implement CA-10's executable DDL contract exactly: typed columns; event-shape CHECKs (STARTED ⇒ hash + FULL content NOT NULL; RESOLVED ⇒ outcome NOT NULL, content NULL); GLOBAL unique index (request_id, post_attempt_seq, event_type) + DROP PARTITION ... UPDATE GLOBAL INDEXES maintenance; monthly interval partitions on occurred_at; SECUREFILE LOB + named tablespaces; local index on idempotency_key; grants + DB audit on reads; §14.1 enablement switch (writes DEFAULT OFF in prod); retention = partition drop per compliance answer; ALSO add post_attempt_seq NUMBER DEFAULT 0 to payment_request (§2.2 - the one §2 column here, coordinated with CA-4).
Tests: T-38 schema slice (INSERT-only enforced; unique pair; event-shape CHECKs; partition drop + global-index usability; app role cannot SELECT). Stop: merged; K-04/RC-02/ST-10 riders unblocked.
```

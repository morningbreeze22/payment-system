...
Objective: implement each §7.2 row as its exact tuple CAS + side effects (anchors, markers, release on definitive rejects). §14.1 rider: ATTEMPT_RESOLVED insert (outcome = §7.2 class verbatim) in the SAME transaction, only on rowCount==1; FAILURE-ISOLATED + switch-gated (insert error → AUDIT-GAP alert, transition proceeds).

...
Objective: implement the §10.4 mapping (view/expression); route dashboards/card label/log/ops reads to it; card returns dimensions + label. Implement the §12 table projection per its contract: row_type REQUEST (one per request) | OBLIGATION_ONLY (no-request placeholder: scope + required-or-blank + "no request created" + derived-exception reason; request fields n/a); obligation context on every row; REQUEST rows pass through required_total_at_creation verbatim (§2.2 amount-series stamp — NULL renders "predates capture"; NEVER computed in the projection); terminal rows visible; removed scopes show CANCELLED context.
Tests: mapping per label row; NEEDS_REVIEW includes blocked_reason; T-31 TABLE cases (placeholder; placeholder→request no-dup; 100+20 = two rows with stamps 100/120; NULL stamp = "predates capture"; mixed active/terminal; CANCELLED context; reappearance). Stop: merged.
```

...
Objective: spec the §14.1 DDL (request_id no-FK, idempotency_key denormalized, post_attempt_seq, event_type ATTEMPT_STARTED|ATTEMPT_RESOLVED, occurred_at UTC monthly-partition key, trigger_source, correlation_id, payload_hash, payload_content + content_ref (dedup: full content ONCE per distinct hash per request), outcome = §7.2 classes verbatim + LEASE_EXPIRED_MAYBE, error/response fields, UNIQUE(request_id, post_attempt_seq, event_type)) and the TWO same-transaction riders: STARTED in the posting claim (K-04 — the reliability rule: no byte leaves without the committed content record); RESOLVED in the episode-ending transaction (RC-02 or ST-10's LEASE_EXPIRED_MAYBE), rowCount==1 only. Fail-safe coupling (journal failure pauses posting; own tablespace + alert); autonomous transactions FORBIDDEN; §16.3 security package (restricted audit role, DB-audited reads, encryption at rest, no lower-env copies, retention per compliance ask); POSTING attempts only.

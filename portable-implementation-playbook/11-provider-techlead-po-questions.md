...
|----|----------|----|----------|-------------|
| Q-01 | BLOCKING | Upstream + UI teams + PO | §18-0 residue — WRITTEN FILINGS ONLY; every design question is settled (PO-9 ANSWERED 2026-07-11: absence = amendment to zero, lifecycle folded round 11 — §4.1 CANCELLED branch; TL-16 ANSWERED round 5): ask 5 IN WRITING — snapshot schema + within-snapshot uniqueness + the ROUND-11 COMPLETE-SET guarantee (each snapshot carries the trade's COMPLETE current settlement set; absence is the cancellation signal, BA-2); ask 8 IN WRITING (round 4 — §18-0(d): fetch-by-id sanctioned, stable versioned ids, consistent reads, IMMUTABILITY, retention ≥ ops/tie SLA); TL-2 step-granularity clause (§12). | B-01 → IN-02 (schema/identity tasks not gated) |
...
| Q-04 | BLOCKING | Provider | §18-1(c): key-retention TTL IN WRITING; is TTL ≥ max row lifetime incl. ops-queue SLA, weekends, holidays, incidents? Verified at the retention edge? (round 10: no local cutoff bounds lifetime — the row can live to the ops SLA) | CT-04; repost_permitted TTL term if short |
...
| Q-06 | CLOSED (round 10) | — | §18-2 ANSWERED by the PO 2026-07-11: the payment engine owns its own calendar; we initiate at any time; residual = the engine's WRITTEN any-time-submission line + late-submission response code (CA-1/Q-08) | — |
| Q-07 | BLOCKING | Platform + tech lead | §18-3: is the apply-platform-verified-outcome operation the confirmed MVP exit (default), or are TL-10 AND TL-5-lookback both affirmed in writing (the only de-scope condition, PO re-confirmation required)? | B-04 → OP-xx |
...
| Q-10 | HIGH | Provider | Query lookback duration — is it ≥ the maximum possible local MAYBE-row lifetime including ops-queue SLA, weekends, holidays, incidents (TL-5 framing — parked rows live days; round 10: no local cutoff caps lifetime)? | §9.3 resolve-via-query viability, §18-3 alternative |
...
| Q-12 | HIGH | Provider / platform | TL-10: can the platform formally REJECT a pending/never-received payment by UETR (or by idempotency key for rows that never received one) so the negative flows back as authoritative feed/query evidence? | §9.3 ops exits; cleaner path than the verified-outcome operation |
...
| Q-20 | MEDIUM | PO / tech lead | Remaining §18 sign-offs: PO-1 ask-then-retry approval; PO-2 query cadence (suggest 2m); PO-3 escalation age (suggest 30m — round 10: no cutoff to clear); PO-4 CLOSED round 10 (no local cutoff exists, §18-2); PO-6 deferred-successor latency acceptance; PO-8 fresh-assembly consequence acceptance; TL-3 RPO/RTO + §5.2 runbook ownership (post-MVP); TL-8 confirmation-age owner+value; TL-9 artifact owners; TL-14 terminal-row archival co-design; TL-15 first-quarter NOT_FOUND-after-trust-age measurement. §19.3/PO-7 (retry-after-reject) = FUTURE. | OB-07 config owners; Section Q risk register |

...
|---|---|---|---|
| Q1 | Read the filed upstream confirmation (ask 5) — it must state snapshot schema + within-snapshot uniqueness as a GUARANTEE ("usually" = FAIL). Run T-01 intake tests incl. the tuple-collision seed. Confirm PO-9 and TL-16 answers recorded in §18 AND the §6.1 fan-out implements them. | signed upstream doc; T-01 run ID; §18 answer entries with date+source | TL + EXT(upstream) |
...
| Q3 | Inspect the deployed cutoff calendar config in the TARGET env: named source + owner, per-currency/market rows incl. holidays, tz-aware repr. Run T-21 spot checks (DST boundary + one holiday per market). Kill the calendar feed in a test env → verify fail direction = blocked per payment_type + alert. | config export; T-21 run ID; stale-calendar drill note | TL + OPS |
| Q4 | Verify OP-01 deployed (restricted role only — attempt EXECUTE with an unauthorized role, must fail), OP-02 suite green on real Oracle, OP-03 drill report SIGNED by the ops owner with real operators + real ticket. Alternative path only with TL-10 ∧ TL-5 letters + PO re-confirmation. | grant listing; T-24 run ID; signed drill report | OPS + TL |
| Q5 | Diff deployed schema vs CA-4 DDL (constraints VALIDATED state, both triggers live, artifact-4 index list present). Run the migration test incl. dual-run (old+new app versions concurrently). EXPLAIN one scanner query per standing scan → each rides its ACTIVE-row-bounded index. | schema diff output; migration test run; captured plans | DEV + DBA |
| Q6 | ST-01..03 suites green; run the L1–L8 legality suite (every illegal write refused by CHECK/trigger — T-15/T-25); audit: grep merged code for UPDATEs on the two tables outside the shared CAS helpers (must be none). | test run IDs; grep/audit note in report format | DEV + TL |
...
| Q13 | As Q3 for the TARGET environment specifically (values, tz db version, holiday load date). | env config export | OPS |
...
| Q15 | = Q4 evidence (procedure suite + drill), filed under the test pack too. | T-24 + drill report | OPS |
...
| Q29 | Run T-33 on real Oracle incl. the refusal cases (MAYBE reject refused at code AND trigger layer; missing ticket / identical approvers / unauthorized role refused) and tie-apply idempotency + latch respect. Audit EXECUTE grants (restricted role only — attempt with an unauthorized role). Open each §15 queue alert definition → its view link resolves. Fire a seeded tie-conflict → inspect that the record carries the canonicalized payload (§6.7 executability). | T-33 run ID; grant listing; seeded tie record capture | OPS + TL |

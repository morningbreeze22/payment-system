...
- **Tests to add:** T-38 schema slice: INSERT-only enforced (no UPDATE/DELETE grants to app/reporting roles); unique-pair constraint; scalar shape CHECK + content trigger reject malformed STARTED/RESOLVED rows (STARTED without content refused by the trigger); partition-drop rehearsal with BOTH global indexes (paj_pk, paj_pair_uq) verified USABLE afterwards; app role cannot SELECT.

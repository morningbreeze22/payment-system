...
- **How to locate:** n/a.
- **Implementation instructions:** implement CA-10's EXECUTABLE DDL CONTRACT exactly (review 7ab31e5 M3 — no improvised types): all columns typed per CA-10; EVENT-SHAPE CHECKs (STARTED ⇒ hash + FULL payload_content NOT NULL, outcome NULL; RESOLVED ⇒ outcome NOT NULL, content NULL); UNIQUE(request_id, post_attempt_seq, event_type) as a GLOBAL unique index; monthly interval partitioning on occurred_at with the maintenance rule DROP PARTITION ... UPDATE GLOBAL INDEXES; SECUREFILE LOB clause + named tablespaces; local index on idempotency_key. GRANTS: INSERT to the application role, SELECT to the restricted audit role ONLY, NO UPDATE/DELETE to anyone; enable DB audit on reads. The §14.1 ENABLEMENT SWITCH config (journal writes DEFAULT OFF in production until the Q30 journal items are evidenced). Retention job = partition drop per the compliance answer (until answered: retain). Document the restore posture per §14.1 (same database — full-DB PITR rewinds it; TSPITR/logical restores of the payment schema do not; the external §14 log is the only restore-surviving record). Also add post_attempt_seq to payment_request per §2.2 (a NUMBER default 0 — the ONE §2 column this task touches, coordinated with CA-4).
...
- **Tests to add:** T-38 schema slice: INSERT-only enforced (UPDATE/DELETE refused for every role); unique-pair constraint; event-shape CHECKs reject malformed STARTED/RESOLVED rows; partition-drop rehearsal WITH global-index usability verified afterwards (UPDATE GLOBAL INDEXES); app role cannot SELECT.

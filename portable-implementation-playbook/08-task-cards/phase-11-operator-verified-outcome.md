...
- **Local code areas to discover:** how the ENTERPRISE ACCESS-MANAGEMENT identities reach the procedure (mechanism DECIDED 2026-07-11, §9.3: each operator has a unique, non-bypassable identity supplied by the enterprise tooling; the procedure verifies distinctness and records both). Discovery confirms the plumbing only. If the enterprise tooling cannot deliver two authenticated identities to the DB layer, BLOCKED — escalate; two free-text strings are non-compliant, never weaken to convention.

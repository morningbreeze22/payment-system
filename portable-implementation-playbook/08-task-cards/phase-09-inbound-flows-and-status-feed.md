...
- **Common mistakes:** >= instead of strictly-newer; tie-differing silently dropped ("upstream resends" is NOT a recovery for ties — §6.7); checking admission but creating the scope in the same transaction WITHOUT holding the trade lock (the race admission exists to close); advancing the trade row inside a block transaction.
- **Completion criteria:** tests green.
- **Stop condition:** merged.
- **Next task:** IN-03.

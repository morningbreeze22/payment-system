...
- **Classification:** MVP normative implementation
- **Purpose:** Section M's dry-run stage: prove the factored model tracks reality before any rule ENFORCEMENT relies on it in production. the comparison table maps CANCELLED EXPLICITLY — legacy display has no such value, so CANCELLED rows are EXPECTED disagreements to be classified, and CANCELLED must NEVER be silently mapped to COMPLETED.
...
- **Next task:** GO-05 (order: rehearsal → pre-cutover authorization → controlled cutover).

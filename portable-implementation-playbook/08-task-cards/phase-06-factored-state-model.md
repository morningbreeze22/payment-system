...
- **How to locate:** D-08.
- **Implementation instructions:** expiry sweep (or claim-time check): CLAIMED + claim_expires_at < DB now → ENRICH: CAS back to READY (clear claim fields); POST: CAS to stage=CONFIRM, stage_state=READY, submission_state=MAYBE_SUBMITTED, clear claim fields, stamp maybe_since (ST-07 helper). No exceptions, no carve-outs. §14.1 rider (FAILURE-ISOLATED, switch-gated): the POST-expiry CAS also INSERTs ATTEMPT_RESOLVED with outcome LEASE_EXPIRED_MAYBE in the SAME transaction (rowCount==1 only — the CAS arbitrates the race with a slow-but-alive worker); an insert error raises the AUDIT-GAP alert and the recovery PROCEEDS (never load-bearing).

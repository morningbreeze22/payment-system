...
- **Prerequisites:** RC-02 (divergent_payload_at written); RC-09 (freeze check — can stub as FROZEN-safe until RC-09 lands). (The former B-03/cutoff prerequisite was RETIRED round 10 — no cutoff term exists, §7.4.) CT-04 REOPEN TERM: if CT-04's typed consequence record reads IMPLEMENTATION_REQUIRED — whenever it lands, including AFTER this card merged under DD-6's late-CT path — this card REOPENS: add the TTL term with its own tests and advance the record to IMPLEMENTED_AND_VERIFIED; the release gates stay blocked meanwhile.
